...
### Ver1.5.0 (2025-01-29) 2025-02-07 バージョン番号の変更なし<br> 
  - 含まれているCodeSystemのうち、以下のURLのCodeSystemのコードdisplay値にミススペルやコード定義不良があったので修正した。
    - urn:oid:1.2.392.200119.6.18100 子宮頸部細胞診 (細胞診婦人科材料)(ベセスダシステム2001)
    - urn:oid:1.2.392.200119.6.19120 あなたはたばこを吸いますか（後期）
    - urn:oid:1.2.392.200119.6.2113 眼底検査(SCOTT分類)
...
### Ver1.5.0 (2025-01-25) <br>
  - JP_Observation_eCheckupGeneralで以下を修正：
    ...
    - Observation.status 検査結果値がなんらかの理由で存在しない場合には、"cancelled"を記述するように修正した。
  - Bundleデータ作例にECG検査一覧グループによる記述例を新たに追加。
  - JP-Coreから派生した汎用ベースプロファイル説明を変更。

### Ver1.4.1 (2025-01-11) <br>
  - JP_Observation_eCheckupGeneral : 
    - 検査結果（valueQuantity、valueString、valueDateTimeの各要素）のプロファイル記述に説明を補。
    - 検査結果の基準値（referenceRange）の下限、上限のValue子要素を仕様書に合わせて多重度1..1をプロファイルに設定した。仕様書の変更はない。

### Ver1.4.0 (2024-12-16) <br>
  - JP_Composition_eCheckupGeneral : 以下のセクションコードが存在する場合にセクションコードエラーとなっていた点について、profileを修正し、それぞれのセクションに対応するスライスのprofileを追加して対応した。
    - 01041/01042 (学校職員健診検査結果/学校職員健診問診結果)
    - 01071/01072 (その他健診検査結果/その他健診問診結果)
    - 01081/01082 (保険者の実施するその他健診検査結果/保険者の実施するその他健診問診結果)
