Instance: Inline-Instance-for-28
InstanceOf: JP_Observation_eCheckupGeneral
Usage: #inline
* meta.lastUpdated = "2024-04-05T13:28:17.239+09:00"
* meta.profile = "http://jpfhir.jp/fhir/eCheckup/StructureDefinition/JP_Observation_eCheckupGeneral"
* status = #final
* category = $JP_SimpleObservationCategory_CS#survey
* code = $specialCheckup_observations_cs#9N506000000000011 "保健指導レベル"
* subject = Reference(urn:uuid:4bc68294-1895-d159-655b-1bc7e83f52f3)
* performer = Reference(urn:uuid:6c4ef273-df66-00ab-c021-d05c5a19e0ba)
* effectiveDateTime = "2020-04-04"

* valueCodeableConcept = urn:oid:1.2.392.200119.6.3001#3 "なし(情報提供)"