...
* identifier ^definition = "この文書Bundleの固定識別子を設定する。'保険医療機関番号10桁＾医療機関内で一意に識別できる粒度の報告ID文字列'。 このうち2つ目要素である「医療機関内で一意に識別できる粒度の報告ID文字列」は、発行年（4桁）、施設内において発行年内で一意となる番号（8桁）をハイフン(-：U+002D)で連結した文字列とする。詳細は仕様書参照のこと。"
* identifier ^comment = ""
...
* entry[organization].resource only  JP_Organization_eCheckupGeneral or JP_OrganizationInsurer_eCheckupGeneral
